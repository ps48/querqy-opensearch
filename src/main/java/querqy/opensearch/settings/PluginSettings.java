...
public class PluginSettings {
    ...
    private static PluginSettings pluginSettingsInstance = null;
     ...
     */
    private final String KEY_PREFIX = "opensearch.querqy";

    /**
     * General settings Key prefix.
     */
    private final String GENERAL_KEY_PREFIX = KEY_PREFIX+".general";

    /**
     * Polling settings Key prefix.
     */
    private final String POLLING_KEY_PREFIX = KEY_PREFIX+".polling";

    /**
     * Access settings Key prefix.
     */
    private final String ACCESS_KEY_PREFIX = KEY_PREFIX+".access";

    /**
     * Operation timeout for network operations.
     */
    private final String OPERATION_TIMEOUT_MS_KEY = GENERAL_KEY_PREFIX+".operationTimeoutMs";

    /**
     * Setting to choose Job lock duration.
     */
    private final String JOB_LOCK_DURATION_S_KEY = POLLING_KEY_PREFIX+".jobLockDurationSeconds";

    /**
     * Setting to choose Minimum polling duration.
     */
    private final String MIN_POLLING_DURATION_S_KEY =  POLLING_KEY_PREFIX+".minPollingDurationSeconds";

    /**
     * Setting to choose Maximum polling duration.
     */
    private final String MAX_POLLING_DURATION_S_KEY =  POLLING_KEY_PREFIX+".maxPollingDurationSeconds";

    /**
     * Setting to choose Maximum number of retries to try locking.
     */
    private final String MAX_LOCK_RETRIES_KEY =  POLLING_KEY_PREFIX+".maxLockRetries";

    /**
     * Setting to choose default number of items to query.
     */
    private final String DEFAULT_ITEMS_QUERY_COUNT_KEY =  GENERAL_KEY_PREFIX+".defaultItemsQueryCount";

    /**
     * Setting to choose admin access restriction.
     */
    private final String ADMIN_ACCESS_KEY =  ACCESS_KEY_PREFIX+".adminAccess";

    /**
     * Setting to choose filter method.
     */
    private final String FILTER_BY_KEY =  ACCESS_KEY_PREFIX+".filterBy";

    /**
     * Setting to choose ignored roles for filtering.
     */
    private final String IGNORE_ROLE_KEY =  ACCESS_KEY_PREFIX+".ignoreRoles";

    /**
     * Default operation timeout for network operations.
     */
    private final Long DEFAULT_OPERATION_TIMEOUT_MS = 60000L;

    /**
     * Minimum operation timeout for network operations.
     */
    private final Long MINIMUM_OPERATION_TIMEOUT_MS = 100L;

    /**
     * Default Job lock duration.
     */
    private final Integer DEFAULT_JOB_LOCK_DURATION_S = 300;

    /**
     * Minimum Job lock duration.
     */
    private final Integer MINIMUM_JOB_LOCK_DURATION_S = 10;

    /**
     * Default Minimum polling duration.
     */
    private final Integer DEFAULT_MIN_POLLING_DURATION_S = 300;

    /**
     * Minimum Min polling duration.
     */
    private final Integer MINIMUM_MIN_POLLING_DURATION_S = 60;

    /**
     * Default Maximum polling duration.
     */
    private final Integer DEFAULT_MAX_POLLING_DURATION_S = 900;

    /**
     * Minimum Maximum polling duration.
     */
    private final Integer MINIMUM_MAX_POLLING_DURATION_S = 300;

    /**
     * Default number of retries to try locking.
     */
    private final Integer DEFAULT_MAX_LOCK_RETRIES = 4;

    /**
     * Minimum number of retries to try locking.
     */
    private final Integer MINIMUM_LOCK_RETRIES = 1;

    /**
     * Default number of items to query.
     */
    private final Integer DEFAULT_ITEMS_QUERY_COUNT_VALUE = 1000;

    /**
     * Minimum number of items to query.
     */
    private final Integer MINIMUM_ITEMS_QUERY_COUNT = 10;

    /**
     * Default admin access method.
     */
    private final String DEFAULT_ADMIN_ACCESS_METHOD = "AllObservabilityObjects";

    /**
     * Default filter-by method.
     */
    private final String DEFAULT_FILTER_BY_METHOD = "NoFilter";

    /**
     * Default filter-by method.
     */
    private List<String> DEFAULT_IGNORED_ROLES =
            Arrays.asList(
                    "own_index",
     ...
     */
    public volatile Long operationTimeoutMs;


    /**
     * Job lock duration
     */
    public volatile Integer jobLockDurationSeconds;

    /**
     * Minimum polling duration
     */
    public volatile Integer minPollingDurationSeconds;

    /**
     * Maximum polling duration.
     */
    public volatile Integer maxPollingDurationSeconds;

    /**
     * Max number of retries to try locking.
     */
    public  volatile Integer maxLockRetries;

    /**
     * Default number of items to query.
     */
    public  volatile Integer defaultItemsQueryCount;

    /**
     * admin access method.
     */
    public  volatile AdminAccess adminAccess;

    /**
     * Filter-by method.
     */
    public  volatile FilterBy filterBy;

    /**
     * list of ignored roles.
     */
    public  volatile List<String> ignoredRoles;
...
//     */
    ...
    private final Integer DECIMAL_RADIX = 10;

    private   final Logger log = LogManager.getLogger(PluginSettings.class);
    private Map<String, String> defaultSettings = new HashMap<>();
    private Setting<Long> OPERATION_TIMEOUT_MS;
    private Setting<Integer> JOB_LOCK_DURATION_S;
    private Setting<Integer> MIN_POLLING_DURATION_S;
    private Setting<Integer> MAX_POLLING_DURATION_S;
    private Setting<Integer> MAX_LOCK_RETRIES;
    private Setting<Integer> DEFAULT_ITEMS_QUERY_COUNT;
    private Setting<String> ADMIN_ACCESS;
    private Setting<String> FILTER_BY;
    private Setting<List<String>> IGNORED_ROLES;
    ...
    private PluginSettings() {
        ...
        if (settings != null){
            operationTimeoutMs = Long.parseLong(settings.get(OPERATION_TIMEOUT_MS_KEY));
            jobLockDurationSeconds = Integer.parseInt(settings.get(JOB_LOCK_DURATION_S_KEY));
            minPollingDurationSeconds = Integer.parseInt(settings.get(MIN_POLLING_DURATION_S_KEY));
            maxPollingDurationSeconds = Integer.parseInt(settings.get(MAX_POLLING_DURATION_S_KEY));
            maxLockRetries = Integer.parseInt(settings.get(MAX_LOCK_RETRIES_KEY));
            defaultItemsQueryCount = Integer.parseInt(settings.get(DEFAULT_ITEMS_QUERY_COUNT_KEY));
            adminAccess = AdminAccess.valueOf(settings.get(ADMIN_ACCESS_KEY));
            filterBy = FilterBy.valueOf(settings.get(FILTER_BY_KEY));
            ignoredRoles = settings.getAsList(IGNORE_ROLE_KEY);
        }
        else{
            operationTimeoutMs = DEFAULT_OPERATION_TIMEOUT_MS;
            jobLockDurationSeconds = DEFAULT_JOB_LOCK_DURATION_S;
            minPollingDurationSeconds = DEFAULT_MIN_POLLING_DURATION_S;
            maxPollingDurationSeconds = DEFAULT_MAX_POLLING_DURATION_S;
            maxLockRetries = DEFAULT_MAX_LOCK_RETRIES;
            defaultItemsQueryCount = DEFAULT_ITEMS_QUERY_COUNT_VALUE;
            adminAccess = AdminAccess.valueOf(DEFAULT_ADMIN_ACCESS_METHOD);
            filterBy = FilterBy.valueOf(DEFAULT_FILTER_BY_METHOD);
            ignoredRoles = DEFAULT_IGNORED_ROLES;
        }
//        assert settings != null;
//        operationTimeoutMs = Optional.of(Long.parseLong(settings.get(OPERATION_TIMEOUT_MS_KEY))).orElse(DEFAULT_OPERATION_TIMEOUT_MS);
...
//        ignoredRoles = Optional.ofNullable(settings.getAsList(IGNORE_ROLE_KEY)).orElse(DEFAULT_IGNORED_ROLES);
        ...
        defaultSettings.put(OPERATION_TIMEOUT_MS_KEY, operationTimeoutMs.toString());
        defaultSettings.put(JOB_LOCK_DURATION_S_KEY, jobLockDurationSeconds.toString());
        defaultSettings.put(MIN_POLLING_DURATION_S_KEY, minPollingDurationSeconds.toString());
        defaultSettings.put(MAX_POLLING_DURATION_S_KEY, maxPollingDurationSeconds.toString());
        defaultSettings.put(MAX_LOCK_RETRIES_KEY, maxLockRetries.toString());
        defaultSettings.put(DEFAULT_ITEMS_QUERY_COUNT_KEY, defaultItemsQueryCount.toString());
        defaultSettings.put(ADMIN_ACCESS_KEY, adminAccess.name());
        defaultSettings.put(FILTER_BY_KEY, filterBy.name());

        OPERATION_TIMEOUT_MS = Setting.longSetting(
                OPERATION_TIMEOUT_MS_KEY,
                Long.parseLong(defaultSettings.get(OPERATION_TIMEOUT_MS_KEY)),
                MINIMUM_OPERATION_TIMEOUT_MS,
                NodeScope, Dynamic);

        JOB_LOCK_DURATION_S = Setting.intSetting(
                JOB_LOCK_DURATION_S_KEY,
                Integer.parseInt(defaultSettings.get(JOB_LOCK_DURATION_S_KEY)),
                MINIMUM_JOB_LOCK_DURATION_S,
                NodeScope, Dynamic
        );
        MIN_POLLING_DURATION_S = Setting.intSetting(
        ...
        MAX_POLLING_DURATION_S = Setting.intSetting(
        ...
        MAX_LOCK_RETRIES = Setting.intSetting(
        ...
        DEFAULT_ITEMS_QUERY_COUNT = Setting.intSetting(
        ...
        ADMIN_ACCESS = Setting.simpleString(
                ADMIN_ACCESS_KEY,
                defaultSettings.get(ADMIN_ACCESS_KEY),
                NodeScope, Dynamic
        );

        FILTER_BY = Setting.simpleString(
                FILTER_BY_KEY,
                defaultSettings.get(FILTER_BY_KEY),
                NodeScope, Dynamic
        );

        IGNORED_ROLES = Setting.listSetting(
                IGNORE_ROLE_KEY,
...
//    private final Setting<Long> OPERATION_TIMEOUT_MS = Setting.longSetting(
//            OPERATION_TIMEOUT_MS_KEY,
//            Long.parseLong(this.defaultSettings.get(OPERATION_TIMEOUT_MS_KEY)),
//            MINIMUM_OPERATION_TIMEOUT_MS,
//            NodeScope, Dynamic);

//    private final Setting<Integer> JOB_LOCK_DURATION_S = Setting.intSetting(
//            JOB_LOCK_DURATION_S_KEY,
//            Integer.parseInt(this.defaultSettings.get(JOB_LOCK_DURATION_S_KEY)),
//            MINIMUM_JOB_LOCK_DURATION_S,
//            NodeScope, Dynamic
//    );

//    private final Setting<Integer> MIN_POLLING_DURATION_S = Setting.intSetting(
//            MIN_POLLING_DURATION_S_KEY,
//            Integer.parseInt(this.defaultSettings.get(MIN_POLLING_DURATION_S_KEY)),
//            MINIMUM_MIN_POLLING_DURATION_S,
//            NodeScope, Dynamic
//    );
//
//    private final Setting<Integer> MAX_POLLING_DURATION_S = Setting.intSetting(
//            MAX_POLLING_DURATION_S_KEY,
//            Integer.parseInt(this.defaultSettings.get(MAX_POLLING_DURATION_S_KEY)),
//            MINIMUM_MAX_POLLING_DURATION_S,
//            NodeScope, Dynamic
//    );
//
//    private final Setting<Integer> MAX_LOCK_RETRIES = Setting.intSetting(
//            MAX_LOCK_RETRIES_KEY,
//            Integer.parseInt(this.defaultSettings.get(MAX_LOCK_RETRIES_KEY)),
//            MINIMUM_LOCK_RETRIES,
//            NodeScope, Dynamic
//    );
//
//    private final Setting<Integer> DEFAULT_ITEMS_QUERY_COUNT = Setting.intSetting(
//            DEFAULT_ITEMS_QUERY_COUNT_KEY,
//            Integer.parseInt(this.defaultSettings.get(DEFAULT_ITEMS_QUERY_COUNT_KEY)),
//            MINIMUM_ITEMS_QUERY_COUNT,
//            NodeScope, Dynamic
//    );
//
//    private final Setting<String> ADMIN_ACCESS = Setting.simpleString(
//            ADMIN_ACCESS_KEY,
//            this.defaultSettings.get(ADMIN_ACCESS_KEY),
//            NodeScope, Dynamic
//    );
//
//    private final Setting<String> FILTER_BY = Setting.simpleString(
//            FILTER_BY_KEY,
//            this.defaultSettings.get(FILTER_BY_KEY),
//            NodeScope, Dynamic
//    );
//
//    private final Setting<List<String>> IGNORED_ROLES = Setting.listSetting(
//            IGNORE_ROLE_KEY,
//            DEFAULT_IGNORED_ROLES,
//            (Function) null,
//            NodeScope, Dynamic
//    );
     ...
     * @param clusterService cluster service instance
     */
    public void addSettingsUpdateConsumer(ClusterService clusterService) {
    ...
    public static PluginSettings getInstance()
    {
        if (pluginSettingsInstance == null)
            pluginSettingsInstance = new PluginSettings();
