...
public class TransportPutRewriterAction extends HandledTransportAction<PutRewriterRequest, PutRewriterResponse> {
    ...
    protected void doExecute(final Task task, final PutRewriterRequest request,
        ...
        indicesClient.prepareGetMappings(Constants.QUERQY_INDEX_NAME).execute(new ActionListener<GetMappingsResponse>() {
            ...
            public void onResponse(final GetMappingsResponse getMappingsResponse) {
                ...
                if (!mappingsVersionChecked) {
                    ...
                    final Map<String, Object> properties = (Map<String, Object>) mappings.get(Constants.QUERQY_INDEX_NAME)
                    ...
                    indicesClient.create(buildCreateQuerqyIndexRequest(indicesClient),
                            new ActionListener<CreateIndexResponse>() {
                                ...
                                public void onResponse(final CreateIndexResponse createIndexResponse) {
                                    LOGGER.info("Created index {}", Constants.QUERQY_INDEX_NAME);
                                    mappingsVersionChecked = true;
                                    try {
                                        saveRewriter(task, request, listener);
                                    } catch (final IOException e) {
    ...
    protected void update1To3(final IndicesAdminClient indicesClient ) throws ExecutionException,
        ...
        final PutMappingRequest request = new PutMappingRequest(Constants.QUERQY_INDEX_NAME).source(
                "{\n" +
                        "    \"properties\": {\n" +
                        "      \"info_logging\": {\n" +
                        "        \"properties\": {\n" +
    ...
    protected CreateIndexRequest buildCreateQuerqyIndexRequest(final IndicesAdminClient indicesClient) {
        ...
        final CreateIndexRequestBuilder createIndexRequestBuilder = indicesClient.prepareCreate(Constants.QUERQY_INDEX_NAME);
        final int numReplicas = settings.getAsInt(Constants.SETTINGS_QUERQY_INDEX_NUM_REPLICAS, Constants.DEFAULT_QUERQY_INDEX_NUM_REPLICAS);
    ...
    private IndexRequest buildIndexRequest(final Task parentTask, final PutRewriterRequest request) throws IOException {
        ...
        final IndexRequest indexRequest = client.prepareIndex(Constants.QUERQY_INDEX_NAME, null, request.getRewriterId())
                .setCreate(false)
