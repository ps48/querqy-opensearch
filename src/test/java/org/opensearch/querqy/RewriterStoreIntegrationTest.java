...
public class RewriterStoreIntegrationTest extends OpenSearchIntegTestCase {
    ...
    public void testThatReplicaSettingForDotQuerqyIndexIsApplied() throws Exception {
        ...
        final GetSettingsResponse idxSettings = client().admin().indices().prepareGetSettings(Constants.QUERQY_INDEX_NAME).get();

        assertNotNull(idxSettings);
        assertEquals(NUM_DOT_QUERY_REPLICAS,
                Integer.parseInt(idxSettings.getIndexToSettings().get(Constants.QUERQY_INDEX_NAME).get("index.number_of_replicas")));
    }
